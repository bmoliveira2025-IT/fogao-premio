...
export default function HeadlinesWidget({ news, nextMatch, className = "" }: HeadlinesWidgetProps) {
    ...
    return (
        <div className={`w-full flex flex-col gap-0 bg-card border resize-none md:rounded-3xl overflow-hidden shadow-2xl ${className}`} style={{ borderColor: 'var(--border-color)' }}>
            ...
            */}
            {nextMatch && (
                <div className="md:hidden border-t border-b bg-background dark:bg-zinc-900/30" style={{ borderColor: 'var(--border-color)' }}>
                    <PremiumNextMatch match={nextMatch} className="rounded-none border-0 shadow-none !bg-transparent" />
                </div>
            )}

            {/* List of Banners (Indices 1-9) */}
            <div className="flex flex-col gap-0 md:gap-2 p-0 md:p-5 bg-card">
                {bannerStories.map((story) => (
                    <Link
                        key={story.id}
                        href={`/news/${story.id}`}
                        className="group relative h-[120px] md:h-[140px] w-full rounded-none md:rounded-xl overflow-hidden border-b md:border border-white/5 last:border-0 md:last:border bg-zinc-900 hover:border-premium-gold/30 transition-all duration-300 block"
                    >
                        {/* Image - Wide crop */}
                        <img
