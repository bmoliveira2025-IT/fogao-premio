...
export default function StandardNewsCard({ article }: { article: StandardArticle }) {
    ...
    return <Link href={`/news/${article.id}`} className="group flex min-h-[132px] gap-4 border-b border-white/[0.06] bg-[#111]/70 p-4 transition-colors hover:bg-[#181818]">
        <div className="min-w-0 flex-1 py-0.5">
            ...
            <h3 className="line-clamp-3 text-base font-extrabold leading-[1.3] text-white group-hover:text-amber-300 sm:text-lg">{article.title?.replace(/\*\*/g, '')}</h3>
            {article.summary && <p className="mt-2 hidden text-sm leading-5 text-zinc-500 line-clamp-2 sm:block">{article.summary}</p>}
        </div>
