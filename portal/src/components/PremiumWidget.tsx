...
import { cn } from '@/lib/utils'; // Assuming utils exists for className merging
...
export default function PremiumWidget({ news, className }: PremiumWidgetProps) {
    return (
        <section className={cn(
            ...
            {/* Header */}
            <div className="relative p-5 pb-2 flex items-center justify-between z-10">
                <div className="flex items-center space-x-2.5">
                    <div className="p-1.5 rounded-full bg-gradient-to-br from-premium-gold to-yellow-600 shadow-lg shadow-premium-gold/20">
                        <Star className="text-white fill-white" size={12} />
                    </div>
                    <span className="text-xs font-black text-foreground uppercase tracking-[0.2em] drop-shadow-sm">
            ...
            {/* Content */}
            <div className="relative p-5 pt-3 space-y-4 z-10">
                {news.length > 0 ? (
                    news.map((item, index) => (
