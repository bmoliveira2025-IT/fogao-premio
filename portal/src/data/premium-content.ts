export const PREMIUM_WALLPAPERS = [
    {
        id: 'wp-1',
        title: 'A Estrela Solitária',
        url: '/wallpapers/star.png',
        thumbnail: '/wallpapers/star.png'
    },
    {
        id: 'wp-2',
        title: 'O Niltão à Noite',
        url: '/wallpapers/stadium.png',
        thumbnail: '/wallpapers/stadium.png'
    },
    {
        id: 'wp-3',
        title: 'Bandeira Alvinegra',
        url: '/wallpapers/flag.png',
        thumbnail: '/wallpapers/flag.png'
    },
    {
        id: 'wp-4',
        title: 'Atmosfera Gloriosa',
        url: '/wallpapers/atmosphere.png',
        thumbnail: '/wallpapers/atmosphere.png'
    },
    {
        id: 'wp-5',
        title: 'Mosaico da Torcida',
        url: '/wallpapers/mosaico.jpg',
        thumbnail: '/wallpapers/mosaico.jpg'
    },
    {
        id: 'wp-6',
        title: 'Vestiário VIP',
        url: '/wallpapers/vestiario.jpg',
        thumbnail: '/wallpapers/vestiario.jpg'
    },
    {
        id: 'wp-7',
        title: 'Gramado Sagrado',
        url: '/wallpapers/gramado.jpg',
        thumbnail: '/wallpapers/gramado.jpg'
    },
    {
        id: 'wp-8',
        title: 'A Taça Libertadores',
        url: '/wallpapers/taca.jpg',
        thumbnail: '/wallpapers/taca.jpg'
    }
];

export const HISTORICAL_MATCHES = [
    {
        id: 'match-yuri-gol',
        title: 'O Golaço Inesquecível',
        subtitle: 'Lance Histórico',
        date: '2025',
        thumbnail: '/wallpapers/goal.png',
        videoId: 'S4vcCVc1tLA',
        description: 'Reviva este momento mágico com a narração emocionante da torcida alvinegra.'
    }
];
